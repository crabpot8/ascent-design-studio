 ...
 ****************************************************************************/
public class ExcelLargeDeploymentConfigTest extends TestCase {

	public void testWorkbookLoad() {
		ExcelDeploymentConfig config = new ExcelDeploymentConfig();
		try {
			config.load(new File("../ascent-data/xls/large.xls"),
					new NetworkBandwidthMinimizingPlanner());
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void testRunProblem() {
		// NetworkBandwidthMinimizingPlanner problem = new
		// NetworkBandwidthMinimizingPlanner();
		...
		for (int i = 0; i < 1; i++) {
			...
			if (best != null && comp.compare(sol, best) > 0)
				best = sol;
			else
				best = sol;
		...
		// sol[i] = i;
		// }
		// DeploymentPlan plan = new DeploymentPlan(problem, new
		// VectorSolution(sol));
		// NetworkGravityOptimizer opt = new NetworkGravityOptimizer();
		// opt.optimize(plan);
		// problem.printSolutionStats(plan.getSolution());
